...
static struct input_dev *sprdtouch_dev;
...
static bool is_keyboard_input(int type, int code)
{
	int i;
								...
								unsigned short packet_len)
{
	...
	while (remain_len > 0) {
		...
		while (frame_remain_len >= KBD_ONE_KEY_SIZE) {
			...
			if (atomic_read(&keyboard_enable)) {
				if (type == 0 && code == 0) {
					// Send SYN events to both devices, the input subsystem will
					// discard empty SYNs anyway.
					input_event(sprdkbd_dev, type, code, value);
					input_event(sprdtouch_dev, type, code, value);
				} else if (is_keyboard_input(type, code)) {
					input_event(sprdkbd_dev, type, code, value);
				} else {
					input_event(sprdtouch_dev, type, code, value);
				}
...
static int sprd_keyboard_register(void)
{
	...
	sprdkbd_dev->evbit[0] = BIT_MASK(EV_KEY) | BIT_MASK(EV_REP) | BIT_MASK(EV_SYN);
	...
	// Touchpad
	sprdtouch_dev = input_allocate_device();
	if (!sprdtouch_dev) {
		input_free_device(sprdkbd_dev);
		return -ENOMEM;
	}

	sprdtouch_dev->name = "Sprd Touchpad";
	sprdtouch_dev->phys = "sprdtouch/input0";
	sprdtouch_dev->id.bustype = BUS_HOST;
	sprdtouch_dev->id.vendor = 0x0001;
	sprdtouch_dev->id.product = 0x0001;
	sprdtouch_dev->id.version = 0x0100;
	sprdtouch_dev->evbit[0] = BIT_MASK(EV_KEY) | BIT_MASK(EV_ABS) | BIT_MASK(EV_SYN);

	__set_bit(BTN_TOUCH, sprdtouch_dev->keybit);
	__set_bit(BTN_LEFT, sprdtouch_dev->keybit);
	input_set_abs_params(sprdtouch_dev, ABS_MT_TOUCH_MAJOR, 0, 255, 0, 0);
	input_set_abs_params(sprdtouch_dev, ABS_MT_WIDTH_MAJOR, 0, 255, 0, 0);
	input_set_abs_params(sprdtouch_dev, ABS_MT_POSITION_Y, 0, DUALUSB_SCREEN_MAX_Y, 0, 0);
	input_set_abs_params(sprdtouch_dev,ABS_MT_POSITION_X,0,DUALUSB_SCREEN_MAX_X,0,0);
	error = input_mt_init_slots(sprdtouch_dev, DUALUSB_MAX_TPADNUM, INPUT_MT_POINTER);
	if (error) {
		pr_err(DUALUSB_PREFIX "failed to input_mt_init_slots: %d\n", error);
	}
	/* error check */
	error = input_register_device(sprdtouch_dev);
	if (error) {
		pr_warn("%s: sprd keyboard register input failed, %d\n",
			__func__, error);
		input_free_device(sprdkbd_dev);
		input_free_device(sprdtouch_dev);
		return error;
	}
...
static void sprd_keyboard_unregister(void)
{
	...
	sprdkbd_dev = NULL;
	input_unregister_device(sprdtouch_dev);
	sprdtouch_dev = NULL;
}
...
static irqreturn_t sprd_hall_isr(int irq, void *dev_id)
{
	...
	if (gpio_get_value(hall_pin) == 0) {
		...
		input_event(hall_dev, EV_KEY, KEY_POWER, 1);
		input_sync(hall_dev);
		input_event(hall_dev, EV_KEY, KEY_POWER, 0);
		input_sync(hall_dev);
